...
```
# Introduction
This document offers an introduction and overview of `r Biocpkg("motifbreakR")`, which allows the biologist to judge whether the sequence surrounding a polymorphism or mutation is a good match to known transcription factor binding sites, and how much information is gained or lost in one allele of the polymorphism relative to another or mutation vs. wildtype. `r Biocpkg("motifbreakR")` is flexible, giving a choice of algorithms for interrogation of genomes with motifs from public sources that users can choose from; these are 1) a weighted-sum, 2) log-probabilities, and 3) relative entropy. `r Biocpkg("motifbreakR")` can predict effects for novel or previously described variants in public databases, making it suitable for tasks beyond the scope of its original design. Lastly, it can be used to interrogate any genome curated within Bioconductor.

`r Biocpkg("motifbreakR")` works with position probability matrices (PPM). PPM are derived as the fractional occurrence of nucleotides A,C,G, and T at each position of a position frequency matrix (PFM). PFM are simply the tally of each nucleotide at each position across a set of aligned sequences. With a PPM, one can generate probabilities based on the genome, or more practically, create any number of position specific scoring matrices (PSSM) based on the principle that the PPM contains information about the likelihood of observing a particular nucleotide at a particular position of a true transcription factor binding site.

This guide includes a brief overview of the [processing flow](#flow), an example focusing more in depth on the [practical](#practical) aspect of using `r Biocpkg("motifbreakR")`, and finally a detailed section on the [scoring methods](#methods) employed by the package.


# Processing overview {#flow}
`r Biocpkg("motifbreakR")` may be used to interrogate SNPs or SNVs for their potential effect on transcription factor binding by examining how the two alleles of the variant effect the binding score of a motif.  The basic process is outlined in the figure below.
...
This section offers an example of how to use `r Biocpkg("motifbreakR")` to identify potentially disrupted transcription factor binding sites due to 701 SNPs output from a `r Biocpkg("FunciSNP")` analysis of Prostate Cancer (PCa) genome wide association studies (GWAS) risk loci.
...
```
The simplest form of a `r Biocpkg("motifbreakR")` analysis is summarized as follows:
...
We use the `r Biocannopkg("SNPlocs.Hsapiens.dbSNP144.GRCh37")` which is the SNP locations and alleles defined in [dbSNP144](http://www.ncbi.nlm.nih.gov/projects/SNP/snp_summary.cgi?view+summary=view+summary&build_id=144) as a source for looking up our rsIDs and `r Biocannopkg("BSgenome.Hsapiens.UCSC.hg19")` which holds the reference sequence for [UCSC genome build hg19](http://www.ncbi.nlm.nih.gov/assembly/2758/). Additional SNPlocs packages are availble from Bioconductor.
...
Sometimes querying a large number of variants from the SNPlocs packages can be very slow. If you find this to be the case, we have implemented querying from [Ensembl BioMart](http://useast.ensembl.org/biomart/martview) using the `r Biocpkg("biomaRt")` package. All that's required is first generating the mart object, and using that instead of a dbSNP object with the `biomart.dataset` argument.

```{r biomart_rsid, message=FALSE, cache=TRUE}
library(biomaRt)
...
A far greater variety of variants may be read into `r Biocpkg("motifbreakR")` via the `snps.from.file` function. In fact `r Biocpkg("motifbreakR")` will work with any organism present as a Bioconductor `r Biocpkg("BSgenome")` package. This includes 113 genomes representing 33 species.
...
Now that we have the three necessary components to run `r Biocpkg("motifbreakR")`:
...
# Methods {#methods}
`r Biocpkg("motifbreakR")` works with position probability matrices (PPM). PPM
...
employed in calls to the `r Biocpkg("motifbreakR")` function via the `method`
argument.
...
equations for [Weighted Sum](#eqn4.1) and [Relative Entropy](#eqn4.2), respectively. `r Biocpkg("motifbreakR")` assumes a
...
all three methods, `r Biocpkg("motifbreakR")` scores and reports the reference
...
`r Biocpkg("motifbreakR")` does not display neutral effects,
...
calculated by `TFMsc2pv`.  This can be (although not always) a very memory and time intensive process if the algorithm doesn't converge rapidly. `calculatePvalue()` may be sped up with the new argument `granularity`, where one may choose speed up calculations by reducing the level of granularity in the p-value calculation. The selecting a level of granularity generates a round matrix where we "Let $M$ be a matrix of real coefficient values ... and let $\epsilon$ be a positive real number. We denote $M_\epsilon$ the round matrix deduced from $M$ by rounding each value by $\epsilon$:"[^TFMPvalue]

$$M_\epsilon(i, x) = \epsilon \lfloor \frac{M(i,x)}{\epsilon} \rfloor$$
$\epsilon$ is called the granularity.

[^TFMPvalue]: Package: [TFMPvalue on CRAN](https://cran.r-project.org/web/packages/TFMPvalue/index.html)
  Paper: [Efficient and accurate P-value computation for Position Weight Matrices](https://doi.org/10.1186/1748-7188-2-15)
  Definition 3 (Round matrix) from the section _Efficient algorithms for computing the P-value without error_
