...
'''
    ...
    script_parts.extend(category_examples)
...
''')
    ...
    install_script = ''.join(script_parts)
...
def create_report(classified_file, output_dir, metadata):
    ...
    # 优化：使用列表推导式构建表格行
    table_rows = [
        f'| {category} | {len(packages)} | {", ".join(packages[:3]) + ("..." if len(packages) > 3 else "")} |'
...
---
*Generated by GitHub Actions on {datetime.now(timezone.utc).strftime('%Y-%m-%d')}*
'''
