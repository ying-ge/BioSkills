			...
			if (!(conf->flag&MPLP_NO_INDEL) && total_depth < max_indel_depth && bcf_call_gap_prep(gplp.n, gplp.n_plp, gplp.plp, pos, bca, ref, rghash) >= 0) {
				...
				for (i = 0; i < n; ++i) {
					...
					for (j = cnt = 0; j < n_plp[i]; ++j) {
						const bam_pileup1_t *p = plp[i] + j;
						if (bam1_qual(p->b)[p->qpos] == 0){bam1_qual(p->b)[p->qpos] = conf->min_baseQ;}
						
						if ((bam1_qual(p->b)[p->qpos] >= conf->min_baseQ)  & (plp[i][j].b->core.qual >= conf->min_mq)) ++cnt;
					}
					printf("\t%d\t", cnt);
					if (n_plp[i] == 0) {
						...
						for (j = 0; j < n_plp[i]; ++j) {
							const bam_pileup1_t *p = plp[i] + j;
							if ((bam1_qual(p->b)[p->qpos] >= conf->min_baseQ) & (plp[i][j].b->core.qual >= conf->min_mq)){
								pileup_seq(plp[i] + j, pos, ref_len, ref);
								//		printf("%s",bam1_qname(p->b));putchar('|');
							}
						}
						if (conf->flag & MPLP_PRINT_POS) {
							...
							for (j = 0; j < n_plp[i]; ++j) {
								const bam_pileup1_t *p = plp[i] + j;
								if ((j >= 0) & (bam1_qual(p->b)[p->qpos] >= conf->min_baseQ) & (plp[i][j].b->core.qual >= conf->min_mq)) {
									putchar(',');
									//	printf("%s",bam1_qname(p->b));putchar('|');
						...
						/*	if (conf->flag & MPLP_PRINT_POS) {
							...
							for (j = 0; j < n_plp[i]; ++j) {
								...
								if ((j >= 0) & (bam1_qual(p->b)[p->qpos] >= conf->min_baseQ) & (plp[i][j].b->core.qual >= conf->min_mq)){
									putchar(',');
									//		printf("%s",bam1_qname(p->b));putchar('|');
									printf("%d", plp[i][j].b->core.l_qseq); // FIXME: printf() is very slow...
						...
						if (conf->flag & MPLP_PRINT_POS) {
							...
							for (j = 0; j < n_plp[i]; ++j) {
								...
								if ((j >= 0) & (bam1_qual(p->b)[p->qpos] >= conf->min_baseQ) & (plp[i][j].b->core.qual >= conf->min_mq)){
									...
									int k;
									for (k = 0; k < p->b->core.n_cigar; ++k){
										printf("%d",bam_cigar_oplen(cigar[k]));
										printf("%c",bam_cigar_opchr(cigar[k]));
